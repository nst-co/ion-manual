...
| 0      | .10.28 |              |                                   |
+--------+--------+--------------+-----------------------------------+
| 1. 0.  | 2014   | Ver1.01      | Ｉ／Ｏステータス追加              |
| 1      | .12.02 |              |                                   |
+--------+--------+--------------+-----------------------------------+
| 1. 0.  | 2015   | Ver1.02      | ＧＩＦファイル表示追加            |
| 2      | .01.30 |              |                                   |
+--------+--------+--------------+-----------------------------------+
| 2. 0.  | 2016   | Ver2.01      | 減衰率追加                        |
| 1      | .08.23 |              |                                   |
+--------+--------+--------------+-----------------------------------+
| 2. 0.  | 2016   | Ver2.02      | オートゲイン追加                  |
| 2      | .08.31 |              |                                   |
+--------+--------+--------------+-----------------------------------+
| 3. 0.  | 2016   | Ver3.01      | 複数                              |
| 1      | .11.07 |              | のマスターから最も近いものを選択  |
+--------+--------+--------------+-----------------------------------+
| 3. 0.  | 2020   | Ver3.02      | 所在地変更                        |
| 2      | .10.08 |              |                                   |
+--------+--------+--------------+-----------------------------------+
| 3. 0.  | 2020   | Ver3.03      | フィルタ設定の説明を追加          |
| 3      | .10.08 |              |                                   |
+--------+--------+--------------+-----------------------------------+

目　次

`1 製品の概要 <#製品の概要>`__ `9 <#製品の概要>`__

`1.1 概要 <#概要>`__ `9 <#概要>`__

`1.2 外観、寸法 <#外観寸法>`__ `9 <#外観寸法>`__

`2 据付 <#据付>`__ `11 <#据付>`__

`2.1 設置条件 <#設置条件>`__ `11 <#設置条件>`__

`2.1.1 設置環境 <#設置環境>`__ `11 <#設置環境>`__

`2.1.2 設置 <#設置>`__ `12 <#設置>`__

`3 仕様 <#仕様>`__ `15 <#仕様>`__

`3.1 本体仕様 <#本体仕様>`__ `15 <#本体仕様>`__

`3.2 機能仕様 <#機能仕様>`__ `15 <#機能仕様>`__

`3.3 各部の仕様 <#各部の仕様>`__ `16 <#各部の仕様>`__

`3.3.1 前面 <#前面>`__ `16 <#前面>`__

`3.3.2 背面 <#背面>`__ `17 <#背面>`__

`3.4 外部入出力 <#外部入出力>`__ `18 <#外部入出力>`__

`3.4.1 入力仕様（シンク型） <#入力仕様シンク型>`__
`18 <#入力仕様シンク型>`__

`3.4.2 入力仕様（ソース型） <#入力仕様ソース型>`__
`18 <#入力仕様ソース型>`__

`3.4.3 出力仕様（シンク型） <#出力仕様シンク型>`__
`19 <#出力仕様シンク型>`__

`3.4.4 出力仕様（ソース型） <#出力仕様ソース型>`__
`19 <#出力仕様ソース型>`__

`4 機能 <#機能>`__ `22 <#機能>`__

`4.1 画面構成 <#画面構成>`__ `22 <#画面構成>`__

`4.1.1 メイン（計測）画面 <#メイン計測画面>`__ `23 <#メイン計測画面>`__

`4.1.2 トレサビ画面 <#トレサビ画面>`__ `27 <#トレサビ画面>`__

`4.1.3 設定画面 <#設定画面>`__ `28 <#設定画面>`__

`4.1.4 登録画面 <#登録画面>`__ `31 <#登録画面>`__

`4.1.5 システムメニュー画面 <#システムメニュー画面>`__
`32 <#システムメニュー画面>`__

`4.1.6 アナログ入力設定画面 <#アナログ入力設定画面>`__
`33 <#アナログ入力設定画面>`__

`4.1.7 フィルタ設計表示 <#フィルタ設計表示>`__
`35 <#フィルタ設計表示>`__

`4.1.8 ＧＩＦファイル表示 <#ｇｉｆファイル表示>`__
`38 <#ｇｉｆファイル表示>`__

`4.1.9 Ｉ／Ｏステータス画面 <#ｉｏステータス画面>`__
`39 <#ｉｏステータス画面>`__

`4.1.10 ファームウェア書き換え <#ファームウェア書き換え>`__
`40 <#ファームウェア書き換え>`__

`4.2 計測手順 <#計測手順>`__ `41 <#計測手順>`__

`5 保守・定期点検 <#保守定期点検>`__ `43 <#保守定期点検>`__

`5.1 保守・定期点検について <#保守定期点検について>`__
`43 <#保守定期点検について>`__

`6 お問い合わせ <#お問い合わせ>`__ `44 <#お問い合わせ>`__
...
|              |                                                       |
|              | 近くに強力な磁場や電                                  |
|              | 磁妨害、静電気放電、無線電波妨害をするものがないこと  |
+--------------+-------------------------------------------------------+
| 振動         | 衝撃、振動が伝わらないこと                            |
+--------------+-------------------------------------------------------+
| 作業スペース | 作業（ティー                                          |
|              | チング、点検、修理）を安全に行えるスペースがあること  |
+--------------+-------------------------------------------------------+
...
+----+-----------------------+----------------------------------------+
| ①  | ＬＡＮ                | Ｌ                                     |
|    | 通信状態ランプ（黄）  | ＡＮ通信が行われている時、点滅します。 |
+----+-----------------------+----------------------------------------+
| ②  | ＬＡＮ                | ＬＡ                                   |
|    | 接続状態ランプ（緑）  | Ｎ回線が接続状態にある時、点灯します。 |
+----+-----------------------+----------------------------------------+
| ③  | 入力信号              | マイクや振動センサからの入力信号を     |
|    | 解析状態ランプ（青）  | 解析している状態にある時、点滅します。 |
+----+-----------------------+----------------------------------------+
...
.. |image2| image:: media/image7.png
   :width: 2.85798in
   :height: 2.25031in
.. |..\\..\\..\\..\\..\\..\\原紙\\danger.gif| image:: media/image2.png
   :width: 0.36458in
   :height: 0.33333in
